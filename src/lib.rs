...
/// ```
pub fn get_strikes(eta: f64, s0: f64, num_x: usize) -> impl IndexedParallelIterator<Item = f64> {
    let b = get_max_k(eta);
    let lambda = get_lambda(num_x, b);
    (0..num_x)
        .into_par_iter()
        .map(move |index| get_k_at_index(b, lambda, s0, index))
}

fn call_aug<T>(v: &Complex<f64>, alpha: f64, cf: T) -> Complex<f64>
where
    T: Fn(&Complex<f64>) -> Complex<f64>,
{
...
where
    T: Fn(&Complex<f64>, f64) -> Complex<f64> + std::marker::Sync,
    S: Fn(f64, usize) -> f64 + std::marker::Sync,
{
    let b = get_max_k(eta);
    let lambda = get_lambda(num_steps, b);
    let fft = Radix4::new(num_steps, false);
    let mut cmpl: Vec<Complex<f64>> = (0..num_steps)
        .into_par_iter()
        .map(|index| {
    ...
    get_strikes(eta, s0, num_steps)
        .zip(output.par_iter().enumerate().map(|(index, &x)| {
            m_out(
                s0 * x.re * (-alpha * get_x(-b, lambda, index)).exp() * eta / (PI * 3.0),
                index,
            )
        }))
        .collect()
...
where
    T: Fn(&Complex<f64>) -> Complex<f64> + std::marker::Sync,
{
    carr_madan_g(
        num_steps,
        eta,
...
mod tests {
    ...
    fn carr_madan_call_returns_correctly() {
        ...
        let eta = 0.25;
        let alpha = 1.5;
        let my_options_price = call_price(num_x, eta, alpha, s0, discount, &bscf);
        let min_n = num_x / 4;
